...
//Также для каждого устройства нужно будет добавлять массив с описанием его хорактеристик, для этого создадим отдельное состояние c пустым массивом - {info, setInfo}
...
//В addInfo вызываем функцию setInfo() которая изменяет состояние компонента. 
//В нее передаем массив, внутри которого, 1м параметром мы копируем/разворачиваем все данные из уже существующего массива, а 2м параметром добавляем новый елемент (где "number: Date.now()" мы будем использовать как уникальный id, чтобы не заморачиваться с созданием другой логики новых и уникальных id). 
//Так, у нас текущий массив заменяется таким же, но уже с новым в нем елементом
//Как пример, логика чем-то похожа на пример ниже:
      ...
      // addInfo()
      // console.log(memo)
//И далее, при помощи {info.map(i =>)}, мы проверяем количество текущий елементов в состоянии [info, setInfo], и в соответствии с количеством елементов в массиве, динамически отрисовываем соответственное количество новых <Row>
