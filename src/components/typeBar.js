...
//"type.id" это ссылка на "this._types" в DeviceStore. И там у каждого типа есть id

//Для того чтобы сделать отображенные типы товаров кликабельными, добавим в добавления в deviceStore логику setSelectedType
...
//Тут как раз таки нам и пригодилось то, что мы ранее указывали в виде ключа id-шник ("key={type.id}") при итерации по массиву
//Когда active === true, то елемент/елементы в ListGroup.Item будут подсвечиваться каким либо цветом
//В итоге, при нажатии на любой тип товара, мы дублируем его в setSelectedType. После чего, если .id добавленного/сохраненного при нажатии кнопки типа товара === .id изначально существующего типа, то он будет подсвечен отдельным цветом
