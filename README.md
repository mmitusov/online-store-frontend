...
Затем в App.js реализуем логику, которая при каждой перезагрузке приложения, через функцию chech() в userAPI.js, выполняла бы запрос на сервер, для проверки пользователя, валидности его токена и потенциального обновления пользовательского токена. Теперь когда мы сразу проверяем пользователя, мы можем корректно отрисовывать навбар в зависимости от того авторизован ли пользователь или нет. И сразу же добавим логику отображения значка загрузки, пока мы ждем ответ от сервера.

Займемся навбаром. Сейчас при выходе с учетки, нас просто редиректит на панель логина. Исправим это и реализуем нормальную функцию по выходу из учетной записи.
