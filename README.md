...
Начнем с пагинации. Для этого, сперва создадим новый компонент и назовем его Pages.js (папка components). Затем скопируем с react-bootstrap компонент pagination и импортируем его туда. После чего, пропишем всю по работе пагинации внутри этого компонента. И после оформления основной визуальной части нашего кампонента, экспортируем его в Shop.js, чтобы он отображался внизу страницы. Далее перейдем в deviceStore.js и создадим там новые поля/состояния которые нам будут нужны для работы логики по пагинации: this._page, this._totalCount, this._limit. И далее переходим в deviceAPI.js, чтобы дописать логику по фетчингу с бекенда нужной нам информации для работы пагинации, с дальнейшем ее сохранением в обновленный deviceStore.js

Теперь остается решить вопрос с подгрузкой конкретного количества устройств. Так как мы хотим ограничить то, сколько устройств может отображаться на каждой странице пагинации. Для этого переходим в компонент Shop.js.
