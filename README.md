...
Ниже будет описана логика перехода на страницы по указаным нами путями-ссылками, дополнительно с описанием того как это работает:
    ...
    Теперь, вместо временной заглушки "const isAuth = false" мы начнем имплементировать стейт менеджер (mobx), который будет следить за изменением состояния всех наших компонентов и контролировать их. Для этого создадим глобальное хранилище "userStore" в папке "store" и начнем прописывать там логику работы стейт-менеджера.

    После чего мы начнем разбираться с тем как это состояние опрокидывать в наши компоненты.