...
И теперь можем перейти к логике по сознанию типов, брендов и устройств на бекенде, через наш фронтернд UI. Для этого в папке HTTP, создадим новый файлик - deviceAPI.js.
