...
Далее реализуем админ панель. Сперва, чтобы на нее переходить, сделаем так, чтобы при нажатии на кнопку "Админ панель" в навбаре, нас пебрасывало на компонент Admin.js (при условии если роль нашего юзера это админ). И теперь можем переходит к реализации данного компонента.
