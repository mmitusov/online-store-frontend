...
И теперь можем перейти к логике по сознанию типов, брендов и устройств на бекенде, через ввод информации на UI нашего фронтернда. Для этого в папке HTTP, создадим новый файлик - deviceAPI.js. И в нем пропишем точки доступа к нашему беку, для получения/загрузки информации по типам, брендам, девайсам. При чем, полученную информацию с бека, мы будем помещать/обновлять в нашем глобальном хранилище deviceStore (mobx). А логикой по получению/сохранению информации на deviceStore, мы воспользуемся в компоненте Shop.js. Ведь он как раз таки отвечает за отображение типов, брендов, девайсов.
